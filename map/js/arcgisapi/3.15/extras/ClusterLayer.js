define([
  ...
  return declare([GraphicsLayer], {
    constructor: function(options) {
      ...
      this.clusterCountField = options.clusterCountField || this.id;
    },
    ...
    add: function(p) {
      ...
      if ( ! clustered ) {
        ...
        p.attributes[this.clusterCountField] = 1;
        this._showCluster(p);
      }
    },
    ...
    _clusterAddPoint: function(p, cluster) {
      ...
      var count, x, y;
      count = cluster.attributes[this.clusterCountField];
      ...
      // increment the count
      cluster.attributes[this.clusterCountField]++;
    ...
    _clusterCreate: function(p) {
      ...
      var clusterCountField = this.clusterCountField;
      // create the cluster
      var cluster = { 
        "x": p.x,
        "y": p.y,
    ...
    _showCluster: function(c) {
      ...
      if (c.attributes[clusterCountField] == 1) {
     
          for (var i = 0; i < this._clusterData.length; i++) {
              if (this._clusterData[i].attributes.clusterId == c.attributes.clusterId) {
                  graphic.symbol = HJSymbol.BuildPictureSymbol(this._clusterData[i].attributes.Image, 0, 0, 5, 36, 36);
                  break;
              }
          }
          //alert(this._clusterData[c.attributes.clusterId]);
         
        return;
      }
     
      // show number of points in the cluster
      var label = new TextSymbol(c.attributes[clusterCountField].toString())
    ...
    _updateLabel: function(c) {
      ...
      if ( label.length == 1 ) {
        ...
        this.remove(label[0]);
        var newLabel = new TextSymbol(c.attributes[this.clusterCountField])
          .setColor(new Color(this._clusterLabelColor))
          .setOffset(0, this._clusterLabelOffset);
        this.add(
          new Graphic(
    ...
    _clusterMeta: function() {
      ...
      arrayUtils.forEach(this._clusters, function(c) {
          count += c.attributes[this.clusterCountField];
      });
