var table;	
$(function(){
//	$.post("http://10.0.109.54:6080/arcgis/rest/services/%E4%B8%93%E9%A2%98/%E8%BF%87%E6%B2%B3%E5%BB%BA%E7%AD%91%E7%89%A9/MapServer/8/query?where=1%3D1&outFields=*&returnGeometry=false&f=pjson",function(json){
	$.post("../sys/js/bridge.json",function(json){
		var data = JSON.parse(json);
//		console.log(data.features);
		$(data.features).each(function(){
			var item = this.attributes;
			var content = "<tr><td>"+item.BRGNAM+"</td><td>"+(item.BRGCOD?item.BRGCOD:"")+"</td>"+
						"<td>"+(item.MEADATE?new Date(item.MEADATE).Format():"")+"</td>"+
						"<td>"+(item.CATBRG?item.CATBRG:"")+"</td>"+
						"<td>"+(item.CONDTN?item.CONDTN:"")+"</td>"+
						"<td>"+(item.MANDEP?item.MANDEP:"")+"</td>"+
						"<td><a href=\"javascript:operate('"+item.BRGNAM+"','"+(item.BRGCOD?item.BRGCOD:"")+"')\" " +
						"role='button' class='btn btn-minier btn-primary'>上传图片</a></td></tr>"
			$("#bridgeTable tbody").append(content);
		});
		
		$('#bridgeTable').DataTable();
	});
	
//	var bridgePath = "http://10.0.109.54:8080/bridgepic/";
	var bridgePath = "http://127.0.0.1:8890/bridgepic/";
...
//			showbuttons: false,
		    ...
		    url: function(params) {
				...
				//图片上传
				$.ajaxFileUpload({
					url : contextpath + "/manage/bridge/uploadbridgePic.do", // 需要链接到服务器地址
					...
					success : function(data, status) {
						...
						//上传完成后添加上传的图片
						$("#choice").attr("src",bridgePath + data.filename)
...
function operate(name,code){
	...
	$.post(contextpath+"/manage/bridge/getBridgePic.do",{bridgename:name,bridgecode:code},function(json){
...
function save(){
	...
	if(src != "/gd_portal/sys/image/noimg.jpg"){
		picpath = src;
		$.post(contextpath+"/manage/bridge/saveBridgePic.do",{picpath:picpath,bridgename:name,bridgecode:code},function(json){
			if(json.state){
