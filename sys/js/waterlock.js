var table;	
$(function(){
	$.post("http://14.23.108.204:6080/arcgis/rest/services/%E4%B8%93%E9%A2%98/%E6%8B%A6%E6%B2%B3%E5%BB%BA%E7%AD%91%E7%89%A9/MapServer/0/query?where=1%3D1&outFields=*&returnGeometry=false&f=pjson",function(json){
//	$.post("../sys/js/waterlock.json",function(json){
		var data = JSON.parse(json);
//		console.log(data.features);
		$(data.features).each(function(){
			var item = this.attributes;
			var content = "<tr><td>"+item.WGTNAM+"</td><td>"+(item.OBJECTID?item.OBJECTID:"")+"</td>"+
						"<td>"+(item.MEADATE?new Date(item.MEADATE).Format():"")+"</td>"+
						"<td>"+(item.CATWGT?item.CATWGT:"")+"</td>"+
						"<td>"+(item.CONDTN?item.CONDTN:"")+"</td>"+
						"<td>"+(item.MANDEP?item.MANDEP:"")+"</td>"+
						"<td><a href=\"javascript:operate('"+item.WGTNAM+"','"+(item.OBJECTID?item.OBJECTID:"")+"')\" " +
						"role='button' class='btn btn-minier btn-primary'>上传图片</a></td></tr>"
			$("#waterlockTable tbody").append(content);
		});
		
		$('#waterlockTable').DataTable();
	});
	
	var waterlockPath = "http://10.0.109.54:8080/waterlockpic/";
...
//			showbuttons: false,
		    ...
		    url: function(params) {
				...
				//图片上传
				$.ajaxFileUpload({
					url : contextpath + "/manage/waterlock/uploadwaterlockPic.do", // 需要链接到服务器地址
					...
					success : function(data, status) {
						...
						//上传完成后添加上传的图片
						$("#choice").attr("src",waterlockPath + data.filename)
...
function operate(name,code){
	...
	$.post(contextpath+"/manage/waterlock/getWaterLockPic.do",{waterlockname:name,waterlockcode:code},function(json){
...
function save(){
	...
	if(src != "/gd_portal/sys/image/noimg.jpg"){
		picpath = src;
		$.post(contextpath+"/manage/waterlock/saveWaterLockPic.do",{picpath:picpath,waterlockname:name,waterlockcode:code},function(json){
			if(json.state){
