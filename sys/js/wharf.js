var table;	
$(function(){
	$.post("http://14.23.108.204:6080/arcgis/rest/services/%E4%B8%93%E9%A2%98/%E4%B8%B4%E6%B2%B3%E5%BB%BA%E7%AD%91%E7%89%A9/MapServer/6/query?where=1%3D1&outFields=*&returnGeometry=false&f=pjson",function(json){
//	$.post("../sys/js/wharf.json",function(json){
		var data = JSON.parse(json);
//		console.log(data.features);
		$(data.features).each(function(){
			var item = this.attributes;
			var content = "<tr><td>"+item.TRMNAM+"</td><td>"+(item.TRMCOD?item.TRMCOD:"")+"</td>"+
						"<td>"+(item.MEADATE?new Date(item.MEADATE).Format():"")+"</td>"+
						"<td>"+(item.CATTRM?item.CATTRM:"")+"</td>"+
						"<td>"+(item.CONDTN?item.CONDTN:"")+"</td>"+
						"<td>"+(item.MANDEP?item.MANDEP:"")+"</td>"+
						"<td><a href=\"javascript:operate('"+item.TRMNAM+"','"+(item.TRMCOD?item.TRMCOD:"")+"')\" " +
						"role='button' class='btn btn-minier btn-primary'>上传图片</a></td></tr>"
			$("#wharfTable tbody").append(content);
		});
		
		$('#wharfTable').DataTable();
	});
	
	var wharfPath = "http://10.0.109.54:8080/wharfpic/";
...
//			showbuttons: false,
		    ...
		    url: function(params) {
				...
				//图片上传
				$.ajaxFileUpload({
					url : contextpath + "/manage/wharf/uploadwharfPic.do", // 需要链接到服务器地址
					...
					success : function(data, status) {
						...
						//上传完成后添加上传的图片
						$("#choice").attr("src",wharfPath + data.filename)
...
function operate(name,code){
	...
	$.post(contextpath+"/manage/wharf/getWharfPic.do",{wharfname:name,wharfcode:code},function(json){
...
function save(){
	...
	if(src != "/gd_portal/sys/image/noimg.jpg"){
		picpath = src;
		$.post(contextpath+"/manage/wharf/saveWharfPic.do",{picpath:picpath,wharfname:name,wharfcode:code},function(json){
			if(json.state){
